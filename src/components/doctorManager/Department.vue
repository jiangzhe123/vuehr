  ...
  <section>
    ...
    <el-col :span="20" class="toolbar" style="padding-bottom: 0px;">
      <el-form :inline="true" :model="filters">
        <el-form-item>
          <el-input v-model="filters.text" placeholder="请输入科室名称查询"></el-input>
        </el-form-item>
        <el-form-item>
    ...
    <el-table :data="departmentList" highlight-current-row v-loading="listLoading" @selection-change="selsChange" style="width: 100%;">
      ...
      <el-table-column align='center' prop="name" label="科室名称" width="120">
      </el-table-column>
  ...
  export default {
    ...
    methods: {
      ...
      getDepartmentList() {
        ...
        this.postRequest('/department/selectAllDepartment',para).then(resp=> {
      ...
      handleDel: function (index, row) {
        this.$confirm('确认删除该科室?', '提示', {
          ...
          this.listLoading = true;
         let para = {ids : [row.id] };
         this.postRequest('/department/deleteDepartmentByIds',para).then(resp=> {
          if (resp.data.success == 1 && resp.status == 200) {
              this.$message({
                message: '删除成功',
                type: 'success'
        ...
        this.$refs.editForm.validate((valid) => {
          if (valid) {
            this.$confirm('确认提交吗？', '提示', {}).then(() => {
              ...
              this.postRequest('/department/updateDepartment',para).then(resp=> {
                if (resp.data.success == 1 && resp.status == 200) {
                    this.$message({
                      message: '编辑成功',
                      type: 'success'
        ...
        this.$refs.addForm.validate((valid) => {
          if (valid) {
            this.$confirm('确认提交吗？', '提示', {}).then(() => {
              ...
              this.postRequest('/department/insertIntoDepartment',para).then(resp=> {
                if (resp.data.success == 1 && resp.status == 200) {
                    this.$message({
                      message: '新增成功',
                      type: 'success'
      ...
      batchRemove: function () {
        ...
        this.$confirm('确认删除选中的['+this.sels.length+']个科室吗？', '提示', {
          type: 'warning'
        }).then(() => {
          this.listLoading = true;
          let para = { ids: ids };
