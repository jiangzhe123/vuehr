  ...
  <section>
    ...
    <el-col :span="20" class="toolbar" style="padding-bottom: 0px;">
      <el-form :inline="true" :model="filters">
        <el-form-item>
          <el-input v-model="filters.text" placeholder="请输入医师资格名称查询"></el-input>
        </el-form-item>
        <el-form-item>
          <el-button type="primary" @click="getQualificationList">查询</el-button>
        </el-form-item>
        <el-form-item>
    ...
    <!--列表-->
    <el-table :data="qualificationList" highlight-current-row v-loading="listLoading" @selection-change="selsChange" style="width: 100%;">
      <el-table-column align='center' type="selection" width="60">
      </el-table-column>
      <el-table-column align='center' type="index" label="编号" width="60">
      </el-table-column>
      <el-table-column align='center' prop="name" label="医师资格名称" width="150">
      </el-table-column>
    ...
    <el-dialog title="编辑" :visible.sync="editFormVisible" :close-on-click-modal="false">
      <el-form :model="editForm" label-width="120px" :rules="editFormRules" ref="editForm">
        <el-form-item label="医师资格名称" prop="name">
  ...
  export default {
    data() {
      return {
        ...
        editFormRules: {
          name: [
            { required: true, message: '请输入医师资格名称', trigger: 'blur' }
          ]
        },
        //编辑初始化界面数据
        editForm: {
    ...
    methods: {
      ...
      getQualificationList() {
        ...
        this.postRequest('/qualification/selectAllQualification',para).then(resp=> {
      ...
      handleDel: function (index, row) {
        this.$confirm('确认删除该医师资格?', '提示', {
          type: 'warning'
        }).then(() => {
          this.listLoading = true;
         let para = {ids : [row.id] };
         this.postRequest('/qualification/deleteQualificationByIds',para).then(resp=> {
          if (resp.data.success == 1 && resp.status == 200) {
              this.$message({
                message: '删除成功',
                type: 'success'
        ...
        this.$refs.editForm.validate((valid) => {
          if (valid) {
            this.$confirm('确认提交吗？', '提示', {}).then(() => {
              ...
              this.postRequest('/qualification/updateQualification',para).then(resp=> {
                if (resp.data.success == 1 && resp.status == 200) {
                    this.$message({
                      message: '编辑成功',
                      type: 'success'
        ...
        this.$refs.addForm.validate((valid) => {
          if (valid) {
            this.$confirm('确认提交吗？', '提示', {}).then(() => {
              ...
              this.postRequest('/qualification/insertIntoQualification',para).then(resp=> {
                if (resp.data.success == 1 && resp.status == 200) {
                    this.$message({
                      message: '新增成功',
                      type: 'success'
      ...
      batchRemove: function () {
        ...
        this.$confirm('确认删除选中的['+this.sels.length+']个医师资格？', '提示', {
          type: 'warning'
        }).then(() => {
          this.listLoading = true;
          let para = { ids: ids };
