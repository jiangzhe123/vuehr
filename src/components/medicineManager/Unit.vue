<template>
  <section>
    ...
    <!--列表-->
    <el-table :data="unitList" highlight-current-row v-loading="listLoading" @selection-change="selsChange" style="width: 100%;">
      <el-table-column align='center' type="selection" width="60">
      </el-table-column>
      <el-table-column align='center' type="index" label="编号" width="60">
      </el-table-column>
      <el-table-column align='center' prop="name" label="药品单位名称" width="150">
      </el-table-column>
    ...
    <el-dialog title="编辑" :visible.sync="editFormVisible" :close-on-click-modal="false">
      <el-form :model="editForm" label-width="130px" :rules="editFormRules" ref="editForm">
        <el-form-item label="药品单位名称" prop="name">
  ...
  export default {
    data() {
      return {
        ...
        editFormRules: {
          name: [
            { required: true, message: '请输入药品单位名称', trigger: 'blur' }
          ]
        },
        //编辑初始化界面数据
        editForm: {
    ...
    methods: {
      ...
      getUnitList() {
        ...
        this.postRequest('/medicineUnit/selectAllUnit').then(resp=> {
          if (resp.data.success == 1 && resp.status == 200) {
            var data = resp.data.result.unitList;
      ...
      handleDel: function (index, row) {
        this.$confirm('确认删除该药品单位名称?', '提示', {
          type: 'warning'
        }).then(() => {
          this.listLoading = true;
         let para = {ids : [row.id] };
         this.postRequest('/medicineUnit/deleteUnitByIds',para).then(resp=> {
          if (resp.data.success == 1 && resp.status == 200) {
              this.$message({
                message: '删除成功',
                type: 'success'
        ...
        this.$refs.editForm.validate((valid) => {
          if (valid) {
            this.$confirm('确认提交吗？', '提示', {}).then(() => {
              ...
              this.postRequest('/medicineUnit/updateUnit',para).then(resp=> {
                if (resp.data.success == 1 && resp.status == 200) {
                    this.$message({
                      message: '编辑成功',
                      type: 'success'
        ...
        this.$refs.addForm.validate((valid) => {
          if (valid) {
            this.$confirm('确认提交吗？', '提示', {}).then(() => {
              ...
              this.postRequest('/medicineUnit/insertIntoUnit',para).then(resp=> {
                if (resp.data.success == 1 && resp.status == 200) {
                    this.$message({
                      message: '新增成功',
                      type: 'success'
      ...
      batchRemove: function () {
        ...
        this.$confirm('确认删除选中的['+this.sels.length+']个药品单位名称？', '提示', {
          type: 'warning'
        }).then(() => {
          this.listLoading = true;
          let para = { ids: ids };
