  ...
  export default {
    ...
    methods: {
      ...
      getSickbedList() {
        ...
        this.postRequest('/sickbed/selectAllNullSickbed',para).then(resp=> {
        ...
        this.$refs.editForm.validate((valid) => {
          if (valid) {
            this.$confirm('确认提交吗？', '提示', {}).then(() => {
              ...
              this.postRequest('/sickbed/updateSickbed',para).then(resp=> {
                if (resp.data.success == 1 && resp.status == 200) {
                    this.$message({
                      message: '编辑成功',
                      type: 'success'
      ...
      getAllPatientByDepartId(val){
        this.postRequest('/sickbed/selectNotSickbedPatient',{departmentId:val}).then(resp=> {
            if (resp.data.success == 1 && resp.status == 200) {
              var data = resp.data.result;
